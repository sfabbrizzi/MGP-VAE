...
def fill_triangular(vector):
    if (len(vector.size()) == 1):
        lenght = vector.size()[0]
        n = int(np.sqrt(0.25 + 2.*lenght) - 0.5)
        ...
        vector = vector.view(vector.size()[0]*vector.size()[1],
                             vector.size()[2])
        lenght = vector.size()[1]
        n = int(np.sqrt(0.25 + 2.*lenght) - 0.5)
        rev_vec = torch.flip(vector, dims=[1])
        ...
        raise Exception(
             "ERROR in fill_triangular function, size of input is : {}".format(
                  str(len(vector.size()))
                      )
                      )
...
def matrix_diag_4d(diagonal):
    diagonal = diagonal.view(diagonal.size()[0]*diagonal.size()[1],
                             diagonal.size()[2],
                             diagonal.size()[3])
...
class Encoder(nn.Module):
    def __init__(self):
        ...
        # layers for MLP
        if (H == 64):
            self.dense1 = nn.Linear(in_features=NUM_FRAMES*128*4*4,
                                    out_features=128)
        elif (H == 32):
            self.dense1 = nn.Linear(in_features=NUM_FRAMES*128*2*2,
                                    out_features=128)
        self.bn1_mlp = nn.BatchNorm1d(num_features=128)

        self.raw_kl1_size = NDIM * NUM_FRAMES * NUM_FRAMES
        self.dense2_1 = nn.Linear(in_features=128,
                                  out_features=self.raw_kl1_size)
        self.bn2_1 = nn.BatchNorm1d(num_features=self.raw_kl1_size)

        self.dense2_2 = nn.Linear(in_features=128,
                                  out_features=(NDIM * NDIM)*((NDIM * NDIM)
                                                              + 1)//2)
        self.bn2_2 = nn.BatchNorm1d(num_features=(NDIM * NDIM) * ((NDIM*NDIM)
                                                                  + 1) // 2)
        self.dense2_3 = nn.Linear(in_features=128,
                                  out_features=NDIM * NUM_FRAMES)
        self.bn2_3 = nn.BatchNorm1d(num_features=NDIM * NUM_FRAMES)
        self.dense2_4 = nn.Linear(in_features=128,
                                  out_features=NDIM * NDIM)
...
class Decoder(nn.Module):
    def __init__(self):
        ...
        self.dense1 = nn.Linear(in_features=NUM_FRAMES*factor,
                                out_features=NUM_FRAMES*8*8*16,
                                bias=True)
        self.bn_dense1 = nn.BatchNorm1d(num_features=NUM_FRAMES*8*8*16)
        self.dense2 = nn.Linear(in_features=NUM_FRAMES*8*8*16,
                                out_features=NUM_FRAMES*8*8*64,
                                bias=True)
        self.bn_dense2 = nn.BatchNorm1d(num_features=NUM_FRAMES*8*8*64)

        self.conv1 = nn.ConvTranspose2d(in_channels=64,
                                        out_channels=64,
                                        kernel_size=3,
                                        stride=1,
                                        padding=1)
        ...
        self.conv3 = nn.ConvTranspose2d(in_channels=64,
                                        ...
                                        padding=1)
        self.bn3 = nn.BatchNorm2d(num_features=32)
        self.conv4 = nn.ConvTranspose2d(in_channels=32,
                                        out_channels=32,
                                        kernel_size=4,
                                        stride=2,
                                        padding=1)
        ...
        self.conv6 = nn.ConvTranspose2d(in_channels=16,
                                        out_channels=16,
                                        kernel_size=4,
                                        stride=2,
                                        padding=1)
        self.bn6 = nn.BatchNorm2d(num_features=16)
        self.conv7 = nn.ConvTranspose2d(in_channels=16,
