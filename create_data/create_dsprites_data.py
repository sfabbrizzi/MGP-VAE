...
#     save_image(im, './dsprites-dataset/images/'+
# (str)(labels[i][1])+'/'+str(labels[i][2])+'_'+str(labels[i][3])+
# '_'+str(labels[i][4])+'_'+str(labels[i][5])+'.png', normalize=True)
...
DATA_DIR = ['./dsprites-dataset/images/0/',
            './dsprites-dataset/images/1/',
            './dsprites-dataset/images/2/']
            ...
            for pos_x in range(0, 32, 8):
                for scale in range(6):
                    ...
                    for frame_idx in range(8):
                        ...
                        image_name = (str(varying_scale)
                                      + '_'
                                      + str(orientation)
                                      + '_'
                                      + str(pos_x)
                                      + '_'
                                      + str(pos_y + 4*frame_idx)
                                      + '.png')

                        video_label_raw.append([shape,
                                                color,
                                                varying_scale,
                                                orientation_degree,
                                                pos_x,
                    ...
                    for i in range(len(video_label_raw)):
                        ...
                        (frame_label_raw[4],
                         frame_label_raw[5]) = (frame_label_raw[5],
                                                frame_label_raw[4])
                        orientation_transpose = 90 - frame_label_raw[3]
                        frame_label_raw[3] = (
                            orientation_transpose
                            if orientation_transpose >= 0
                            else 360 + orientation_transpose
                        )
                    ...
                    for i in range(len(video_label_transpose)):
                        video_label_transpose_flip.append(
                            video_label_transpose[-1-i]
                            )

                    video_label = (video_label_raw
                                   + video_label_flip
                                   + video_label_transpose
                                   + video_label_transpose_flip)
                    ...
                    for frm_idx in range(arr.shape[0]):
                        ...
                        save_img(os.path.join(
                            './test', str(scale)+'_'+str(frm_idx)+'.png'
                            ),
                                 frame,
                    ...
                    for frm_idx in range(arr_flip.shape[0]):
                        ...
                        save_img(
                            os.path.join(
                             './test',
                             'flip_'+str(scale)+'_'+str(frm_idx)+'.png'
                             ),
                            frame,
                            data_format='channels_first')
                    ...
                    for frm_idx in range(arr_transpose.shape[0]):
                        ...
                        save_img(
                            os.path.join(
                             './test'
                             'transpose_'+str(scale)+'_'+str(frm_idx)+'.png'),
                            frame,
                            data_format='channels_first')
...
for d in DATA_DIR:
    ...
    shape = (int)(d.split('/')[3])
    for color in range(4):
        # remove small scales as they are bad for training
        for scale in range(4, 6):
            for pos_x in range(0, 32, 2):
                for orientation in range(40):
                    ...
                    for frame_idx in range(8):
                        ...
                        varying_orientation_degree = (varying_orientation
                                                      * 360
                                                      / 39)

                        image_name = (str(scale)
                                      + '_'
                                      + str(varying_orientation)
                                      + '_'
                                      + str(pos_x)
                                      + '_'
                                      + str(pos_y + 4*frame_idx)
                        ...
                        video_label_raw.append(
                            [shape,
                             color,
                             scale,
                             varying_orientation_degree,
                             pos_x,
                             pos_y + 4 * frame_idx]
                             )
                    ...
                    for i in range(len(video_label_raw)):
                        ...
                        (frame_label_raw[4],
                         frame_label_raw[5]) = (frame_label_raw[5],
                                                frame_label_raw[4])
                        orientation_transpose = 90 - frame_label_raw[3]
                        frame_label_raw[3] = (orientation_transpose
                                              if orientation_transpose >= 0
                                              else 360 + orientation_transpose)
                    ...
                    for frm_idx in range(arr.shape[0]):
                        ...
                        save_img(
                            os.path.join(
                                './test',
                                str(orientation)+'_'+str(frm_idx)+'.png'
                                ),
                            frame,
                            data_format='channels_first')
