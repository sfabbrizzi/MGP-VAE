...
##################################################################
# script to generate moving mnist video dataset (frame by frame)
# as described in [1] arXiv:1502.04681 - Unsupervised Learning
# of Video Representations Using LSTMs
#     Srivastava et al
# by Tencia Lee
...
def arr_from_img(im, shift=0):
    ...
    arr = np.asarray(arr, dtype=np.float32).reshape((h, w, c))
    return arr.transpose(2, 1, 0) / 255. - shift
...
def generate_moving_mnist(shape=(64, 64),
    ...
    for seq_idx in range(seqs):
        ...
        veloc = [
            (v*math.cos(d), v*math.sin(d))
            for d, v in zip(direcs, speeds)]
        mnist_images = [
            Image.fromarray(
                get_picture_array(mnist, r, shift=0)
        ...
        for frame_idx in range(seq_len):
            ...
            for i, canv in enumerate(canvases):
                canv.paste(mnist_images[i],
                           tuple(map(lambda p: int(round(p)),
                                     positions[i])))
...
def main(dest,
    ...
    if filetype == 'hdf5':
        ...
        for i in range(dat.shape[0]):
            Image.fromarray(
                get_picture_array(dat, i, shift=0)).save(
                    os.path.join(dest, '{}.jpg'.format(i))
                    )
...
if __name__ == '__main__':
    ...
    parser.add_argument('--seq_len',
                        type=int,
                        dest='seq_len',
                        default='20')  # length of each sequence
    parser.add_argument('--seqs',
                        type=int,
                        dest='seqs',
                        default='10000')  # number of sequences to generate
    parser.add_argument('--num_sz',
                        type=int,
                        dest='num_sz',
