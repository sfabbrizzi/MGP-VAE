...
def KL_loss_L1_without_mean(sigma_p_inv, sigma_q, mu_q, det_p, det_q):
    ...
    l1 = torch.einsum('kij,mkji->mk',
                      sigma_p_inv,
                      sigma_q)  # tr(sigma_p_inv sigma_q)
    l2 = torch.einsum('mki,mki->mk',
                      mu_q,
                      torch.einsum('kij,mkj->mki',
                                   sigma_p_inv,
                                   mu_q))  # <mu_q, sigma_p_inv, mu_q>
    loss = torch.sum(
        l1 + l2 + torch.log(det_p) - torch.log(det_q),
        dim=1
        )  # KL divergence b/w two Gaussian distri
    return loss


def KL_loss_L1(sigma_p_inv, sigma_q, mu_q, mu_p, det_p, det_q):
    # sigma_p_inv: (n_dim, n_frames, n_frames),
    # det_p: (d)
    # sigma_q: (batch_size, n_dim, n_frames, n_frames),
    # mu_q: (batch_size, d, nlen)

    l1 = torch.einsum('kij,mkji->mk',
                      sigma_p_inv, sigma_q)  # tr(sigma_p_inv sigma_q)
    l2 = torch.einsum('mki,mki->mk',
                      mu_p - mu_q,
                      torch.einsum('kij,mkj->mki',
                                   sigma_p_inv,
...
if (__name__ == '__main__'):
    ...
    for epoch in range(START_EPOCH, END_EPOCH):
        ...
        for iteration in range(len(dataset)//BATCH_SIZE):
            ...
            sigma_q1 = torch.einsum('ijkl,ijlm->ijkm',
                                    KL1,
            ...
            if (ZERO_MEAN_FEA):
                ...
                kl_loss1 = KL_loss_L1_without_mean(sigma_p_inv,
                                                   sigma_q1,
                                                   mul1_transpose,
                                                   det_p,
                                                   det_q1)
            ...
            if (iteration % 200 == 0 and iteration != 0):
                print('Epoch : ', epoch,
                      ...
                      ', Image Loss : ', torch.mean(img_loss).item(),
                      ', KL Div. Loss : ', torch.mean(kl_loss).item())
                epoch_loss += total_loss.item()

        # write to tensorboard
        writer.add_scalar('Total loss',
                          total_loss.data.storage().tolist()[0],
                          epoch)
        writer.add_scalar('KL-Divergence loss',
                          kl_loss.data.storage().tolist()[0],
                          epoch)
        writer.add_scalar('Image loss',
                          img_loss.data.storage().tolist()[0],
                          epoch)
        ...
        save_image(original_sample,
                   OUTPUT_PATH + '/epoch={}_original.png'.format(str(epoch)),
                   nrow=NUM_FRAMES,
                   normalize=True)
        save_image(decoded_sample,
                   OUTPUT_PATH + '/epoch={}_recon.png'.format(str(epoch)),
                   nrow=NUM_FRAMES,
                   normalize=True)
        ...
        if epoch_loss < lowest_loss:
            ...
            # save checkpoints
            torch.save(
                encoder.state_dict(), os.path.join('checkpoints', ENCODER_SAVE)
                )
            torch.save(
                decoder.state_dict(), os.path.join('checkpoints', DECODER_SAVE)
                )
            print('Model Saved! Epoch loss at {}'.format(lowest_loss))

            encoder_test.load_state_dict(
                torch.load(os.path.join('checkpoints', ENCODER_SAVE))
                )
            decoder_test.load_state_dict(
                torch.load(os.path.join('checkpoints', DECODER_SAVE))
                )
            ...
            X1_v2, KL1_v2, muL1_v2, det_q1_v2 = encoder_test(video2,
                                                             BATCH_SIZE=1)
            dec_v2 = decoder_test(X1_v2, BATCH_SIZE=1)

            # visualize style transfer
            plot_training_images(video1,
                                 video2,
                                 dec_v1,
                                 dec_v2,
                                 X1_v1,
