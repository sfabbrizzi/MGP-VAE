...
from flags import (NUM_SAMPLE_GEO_OUTPUT,
                   NDIM,
                   NUM_FRAMES,
                   H,
                   W,
...
def find_energy(z0, z1, z2):
    ...
    a2 = (decoder(torch.cat([Z_remaining,
                             z2.view(1, NUM_SAMPLE_GEO_OUTPUT, NDIM)], 1))
          - 2*decoder(torch.cat([Z_remaining,
                                 z1.view(1, NUM_SAMPLE_GEO_OUTPUT, NDIM)], 1))
          + decoder(torch.cat([Z_remaining,
                               z0.view(1, NUM_SAMPLE_GEO_OUTPUT, NDIM)], 1))
          ).view(1, 1, NUM_FRAMES, H, W)
...
def find_etta_i(z0, z1, z2):
    ...
    dec = decoder(
        torch.cat([Z_remaining, z1.view(1, NUM_SAMPLE_GEO_OUTPUT, -1)], 1)
        )
    v = (decoder(
        torch.cat([Z_remaining, z2.view(1, NUM_SAMPLE_GEO_OUTPUT, -1)], 1)
        )
         - 2*dec
         + decoder(
            torch.cat([Z_remaining, z0.view(1, NUM_SAMPLE_GEO_OUTPUT, -1)], 1))
        )
    ...
    for i in range(1, N):
        delta_e += compute_norm(
            find_energy(z_collection[i-1].view(NUM_SAMPLE_GEO_OUTPUT, -1),
                        z_collection[i].view(NUM_SAMPLE_GEO_OUTPUT, -1),
                        z_collection[i+1].view(NUM_SAMPLE_GEO_OUTPUT, -1))
                )

    # energy_arr: a float tensor of size = (num_frames)
    # where each index corresponds to energy of each point
...
def geodesic_interpolation(z_collection):
    ...
    while True:
        ...
        for i in range(1, len(z_collection) - 1):
            etta_i = find_etta_i(z_collection[i - 1],
                                 z_collection[i],
                                 z_collection[i + 1])
            etta_i = torch.nn.functional.normalize(etta_i, p=2, dim=1)
            e1 = STEP_SIZE * etta_i

            # update latent points in the direction
            # of decreasing gradient of energy
            z_collection[i] = (z_collection[i]
                               - e1.view(1, NUM_SAMPLE_GEO_OUTPUT, -1))
...
if (__name__ == '__main__'):
    ...
    for epoch in range(NUM_EPOCHS_GEO):
        ...
        for iteration in range(len(dataset) // BATCH_SIZE):
            ...
            # latent space loss
            z_allfeature = linear_interpolation(output,
                                                X1[:,
                                                   -NUM_SAMPLE_GEO_OUTPUT:,
                                                   :])
            ...
            decoded_pred = decoder(
                torch.cat([Z_remaining, output.view(1,
                                                    NUM_SAMPLE_GEO_OUTPUT,
                                                    NDIM)], dim=1))
            ...
            # printing loss
            if (iteration % 100 == 0):
                print('Loss at Epoch {}, Iteration {} is {}'.format(str(epoch),
                      str(iteration),
                      str(loss.item())))

            # saving model
            torch.save(prediction_model.state_dict(),
                       os.path.join('checkpoints', 'prediction_model'))
