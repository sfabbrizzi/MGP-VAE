# this file contains all necessary functions to
# generate covariance matrices for all relevant GP
...
def gen_BB2_cov(nlen, fac=0.1):
    ...
    for i in range(nlen):
        for j in range(i):
            sigma_p[i, j] = ((1 - i/nlen)*(1 - j/nlen)
                             + (i*j)/(nlen*nlen)
                             + (fac*fac)*(j - (i*j)/nlen))
            sigma_p[j, i] = sigma_p[i, j]
        sigma_p[i, i] = ((1 - i/nlen)*(1 - i/nlen)
                         + (i*i)/(nlen*nlen)
                         + (fac*fac)*i*(1 - i/nlen))
...
def gen_OU_cov(nlen, fac=0.1, alpha=1):
    ...
    for i in range(nlen):
        for j in range(i):
            sigma_p[i, j] = ((1 + b * (numpy.exp(2 * alpha * j) - 1))
                             * numpy.exp(-alpha * (i + j)))
            sigma_p[j, i] = sigma_p[i, j]
...
def gen_frac_cov(H, nlen, fac=0.3):
    ...
    for i in range(nlen):
        for j in range(i):
            sigma_p[i, j] = 1 + (fac * fac) * 0.5 * (i**(2*H)
                                                     + j**(2*H)
                                                     - (i - j)**(2*H))
            sigma_p[j, i] = sigma_p[i, j]
...
# Slow features (taken from paper: Disentangling Space and Time
# in Video with Hierarchical Variational Auto-encoders)
...
def covariance_function(p_type, nlen, H=None):
    ...
    if (p_type == 'frac'):
        ...
        raise Exception('Gaussian Process {}, H = {} not implemented'.format(
            p_type,
            H)
             )
