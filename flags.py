# this file contains all arguments required
# for training and evaluation of MGP VAE
...
parser.add_argument('--dataset',
                    ...
                    help=("dataset to be used for training/testing "
                          "(moving_mnist/dsprites/dsprites_color)"))
...
parser.add_argument('--num_dim',
                    type=int,
                    default=2,
                    help="total dimension of latent space")
parser.add_argument('--num_fea',
                    type=int,
                    default=2,
...
parser.add_argument('--fea',
                    type=list,
                    default=['frac_0.1', 'frac_0.9'],
                    help=("All Gaussian processes as a list "
                          "(options = frac_0.1, frac_0.9, bb, bb2, ou)"))
parser.add_argument('--zero_mean_fea',
                    type=bool,
                    default=False,
                    help="Non-zero Mean for all GPs (False = zero mean)")
parser.add_argument('--mean_fea_s',
                    type=list,
                    default=[-2, -2, -2, -2, -2],
                    help="Starting Means of all Gaussian processes")
parser.add_argument('--mean_fea_e',
                    type=list,
                    default=[2, 2, 2, 2, 2],
                    help="Ending Means of all Gaussian processes")
parser.add_argument('--keep_rho',
                    type=bool,
                    default=False,
                    help="use rho for create_path fuction")

parser.add_argument('--beta',
                    type=float,
                    default=2.0,
                    help="coeff. of kl_loss in total_loss")
...
parser.add_argument('--encoder_save',
                    type=str,
                    default='encoder',
                    help="model save for encoder")
parser.add_argument('--decoder_save',
                    type=str,
                    default='decoder',
...
parser.add_argument('--is_training',
                    type=bool,
                    default=True,
                    help="flag to indicate if it is training or inference.")

# visualization
parser.add_argument('--num_points_visualization',
                    type=int,
                    default=6,
                    help="number of videos to plot in visualization")

# geodesic prediction
parser.add_argument('--num_epochs_geodesic',
                    type=int,
                    default=10,
                    help="number of epochs for geodesic prediction")
parser.add_argument('--max_geo_iter',
                    type=int,
                    default=5,
                    help=("number of maximum iterations "
                          "for geodesic prediction"))
parser.add_argument('--num_samples_input',
                    type=int,
                    default=5,
                    help=("number of frames that the prediction "
                          "network takes as input"))
parser.add_argument('--num_samples_output',
                    type=int,
                    default=1,
                    help=("number of frames that the prediction "
                          "network gives as output"))
parser.add_argument('--latent_weight',
                    type=float,
                    default=0.5,
                    help=("weight of geodesic latent space loss "
                          "in total geodesic prediction loss function"))
parser.add_argument('--step_size',
                    type=float,
                    default=0.1,
                    help="step size for geodesic loss gradient update")
parser.add_argument('--threshold',
                    type=float,
                    default=0.1,
                    help="geodesic energy threshold")
...
if (FLAGS.dataset == 'moving_mnist'):
    ...
    DATA_PATH = ('./data/dsprites/'
                 'trainset_dsprites_data_color_with_motion_test_data.h5')
else:
